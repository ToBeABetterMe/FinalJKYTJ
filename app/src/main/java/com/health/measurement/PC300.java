...
public class PC300 {
	...
	public static final byte[] COMMAND_BP_NEWBABY = { (byte) 0xaa, 0x55, 0x40,
			0x03, 0x03, 0x01, -37 };
	public static final byte[] COMMAND_BP_CHILD = { (byte) 0xaa, 0x55, 0x40,
			0x03, 0x03, 0x02, 57 };
	public static final byte[] COMMAND_BP_ADULT = { (byte) 0xaa, 0x55, 0x40,
			0x03, 0x03, 0x00, -123 };
	...
	public static final byte TOKEN_GLU = (byte) 0x73;// 上传血糖令牌
	...
	public Float getGlu(byte[] data) {
		int flag = (data[5] >> 4) & (3);
		if (flag == 0) {
			int res = Integer.parseInt(printHexString(data[7])) + data[6] * 100;
	...
	/**
	 * 将读到的多条数据分割，返回每个类型最新的合法的数据，如有多条血压当前值得数据， 只保留最后一条合法的当前血压数据
	 * 
	 * @param buffer
	 * @return
	 */
	...
	public static void main(String[] args) {
		...
		testCrc.printDataWithCrc(COMMAND_BP_NEWBABY, "COMMAND_BP_START");
		testCrc.printDataWithCrc(COMMAND_BP_CHILD, "COMMAND_BP_STOP");
		testCrc.printDataWithCrc(COMMAND_BP_ADULT, "COMMAND_BETTERY");
...
//				frame.getFramNum() + ":" + Arrays.toString(frame.ecg));
	...
	public String saveEcgFrams(EcgFrame frame) {
