...
/**
 * 多选框的edittext,可以设置多个固定的选项和一个可以编辑的选项 其他项可以编辑，通过setFixItems(String[]
 * items)设置固定的选项， 通过setEditableItem(String item)设置可以编辑的项 选中多个后，每个项以跳格分隔拼接成字符串
 * 
 * @author jiqunpeng
 * 
...
public class MultiChoiceEditText2 extends ChoiceEditText {

	public MultiChoiceEditText2(Context context, AttributeSet attrs,
			int defStyle) {
	...
	public MultiChoiceEditText2(Context context) {
		super(context);
	}
	...
	private class SelectDialog extends AlertDialog.Builder {
		...
		private View initDialogContent() {
			...
			if (other != null) {// 需要显示其他这一行
				...
				root.findViewById(R.id.other_layout).setVisibility(View.GONE);
			setSelectedItem();
			return root;
		}
