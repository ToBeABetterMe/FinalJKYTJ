package com.c102c.app.bluetooth;
...
public class BluetoothService {
	...
	public static BluetoothService getService(Handler handler, boolean isAsyn) {
		if (lastInstance != null) {
			if (BluetoothService.lastInstance.handler == handler)
	...
	public synchronized void connect(BluetoothDevice device) {
		...
		if (DEBUG)
			//Log.i(TAG, "connectThread.shutdown(1)");
		// 断开已经建立好连接
		if (connectedThread != null) {
		...
		if (DEBUG)
			//Log.i(TAG, "connectedThread.shutdown(2)");
		try {
...
//		Message msg2 = Message.obtain();
	...
	private class ConnectThread extends Thread {
		...
		public ConnectThread(BluetoothDevice device) throws SecurityException,
			...
			if (DEBUG)
				//Log.i(TAG, "ConnectThread(): " + device.getName()+"--"+device.getAddress());
			this.device = device;
			if (btAdapter.getState() == BluetoothAdapter.STATE_OFF)
				btAdapter.enable();// 打开蓝牙
			...
			try {
				if (sdkVersion >= 10) {// 10以上的使用不安全连接
					...
					tempSocket = device
							.createRfcommSocketToServiceRecord(MY_UUID);
					//Log.i(TAG, tempSocket.isConnected()+"");
	...
	private class ConnectedThread extends Thread {
		...
		public void run() {
			...
			while (!stop & isAsyn) {// 异步方式下，不停的读数据//是不是写错了？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？
				read();
			}
			while (!stop && !isAsyn) {
				synchronized (this) {
